...
impl Config<Instance1> for Test {
	...
	type ProposalCloseDelay = ConstU64<2>;
	type MaxProposals = MaxProposals;
	type MaxMembers = MaxMembers;
	type DefaultVote = PrimeDefaultVote;
	type FoundationAccountsProvider = FoundationAccountsProvider<Test>;
	type WeightInfo = ();
...
impl ExtBuilder {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		let mut ext: sp_io::TestExternalities = RuntimeGenesisConfig {
			...
			collective_majority: pallet_collective_mangata::GenesisConfig {
...
fn close_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_noop!(
			Collective::close(RuntimeOrigin::signed(3), hash, 0, proposal_weight, proposal_len),
...
#[test]
fn proposal_close_delay_avoided_by_foundation_account_works() {
	ExtBuilder::default().build_and_execute(|| {
		let proposal = make_proposal(42);
		let proposal_len: u32 = proposal.using_encoded(|p| p.len() as u32);
		let proposal_weight = proposal.get_dispatch_info().weight;
...
fn close_with_prime_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			System::events(),
			vec![
				record(RuntimeEvent::Collective(CollectiveEvent::MembersChanged {
					new_members: vec![1, 2, 3]
				})),
				record(RuntimeEvent::Collective(CollectiveEvent::PrimeSet { new_prime: Some(3) })),
				record(RuntimeEvent::Collective(CollectiveEvent::Proposed {
					account: 1,
					proposal_index: 0,
					proposal_hash: hash,
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			vec![
				...
				record(RuntimeEvent::Collective(CollectiveEvent::PrimeSet { new_prime: Some(1) })),
				record(RuntimeEvent::Collective(CollectiveEvent::Proposed {
					account: 1,
					proposal_index: 0,
					proposal_hash: hash,
...
#[test]
fn disapprove_proposal_with_foundation_account_works() {
	ExtBuilder::default().build_and_execute(|| {
		let proposal = make_proposal(42);
		let proposal_len: u32 = proposal.using_encoded(|p| p.len() as u32);
		let hash: H256 = proposal.blake2_256().into();
...
fn genesis_build_panics_with_too_many_members() {
	...
	pallet_collective_mangata::GenesisConfig::<Test> {
