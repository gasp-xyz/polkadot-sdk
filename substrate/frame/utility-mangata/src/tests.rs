...
impl pallet_collective_mangata::Config<CouncilCollective> for Test {
	type RuntimeOrigin = RuntimeOrigin;
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	pallet_collective_mangata::GenesisConfig::<Test, Instance1> {
		members: vec![1, 2, 3],
...
fn batch_all_filters_disallowed_with_signed() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			...
			DispatchErrorWithPostInfo {
				post_info: PostDispatchInfo {
					actual_weight: Some(Weight::from_parts(23417608, 0)),
					pays_fee: Pays::Yes
				},
				error: frame_system::Error::<Test>::CallFiltered.into()
			}
...
fn batch_works_with_council_origin() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Council(pallet_collective_mangata::Event::Executed {
			proposal_hash: hash,
			result: Ok(()),
		}));
	})
...
fn batch_all_works_with_council_origin() {
	new_test_ext().execute_with(|| {
		assert_ok!(Utility::batch_all(
			RuntimeOrigin::from(pallet_collective_mangata::RawOrigin::Members(3, 3)),
