...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn sudo(
			...
			let res = call.clone().dispatch_bypass_filter(RawOrigin::Root.into());
		...
		pub fn sudo_as(
			...
			let res = call.clone().dispatch_bypass_filter(RawOrigin::Signed(who.clone()).into());
		...
		pub fn remove_key(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			...
			alert_log!(info, "A remove_key action was performed!");
