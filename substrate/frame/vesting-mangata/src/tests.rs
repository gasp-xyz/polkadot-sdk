...
use crate::mock::{Balance, Balances, ExtBuilder, System, Test, Vesting, TKN};
...
where
	...
	u32: EncodeLike<TokenIdOf<T>>,
{
	// Its ok for this to fail because the user may already have no schedules.
	let _result = Vesting::vest(Some(account).into(), TKN);
	assert!(!<VestingStorage<T>>::contains_key(account, TKN));
}
...
fn check_vesting_status() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting(&1, TKN).unwrap(), vec![user1_vesting_schedule]); // Account 1 has a vesting schedule
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![user2_vesting_schedule]); // Account 2 has a vesting schedule
		assert_eq!(Vesting::vesting(&12, TKN).unwrap(), vec![user12_vesting_schedule]); // Account 12 has a vesting schedule

		// Account 1 has only 128 units vested from their illiquid ED * 5 units at block 1
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(128 * 9));
		// Account 2 has their full balance locked
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(user2_free_balance));
		// Account 12 has only their illiquid funds locked
		assert_eq!(Vesting::vesting_balance(&12, TKN), Some(user12_free_balance - ED * 5));

		System::set_block_number(10);
		assert_eq!(System::block_number(), 10);

		// Account 1 has fully vested by block 10
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(0));
		...
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(0)); // Account 1 is still fully vested, and not negative
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(0)); // Account 2 has fully vested by block 30
		assert_eq!(Vesting::vesting_balance(&12, TKN), Some(0)); // Account 2 has fully vested by block 30
...
fn check_vesting_status_for_multi_schedule_account() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(4u64, 2, sched1, TKN));
		...
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched1]);
		// sched1 has free funds at block #1, but nothing else.
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(free_balance - sched1.per_block()));
		...
		assert_ok!(Vesting::do_vested_transfer(4u64, 2, sched2, TKN));
		...
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(0));
		// Since we have not called any extrinsics that would unlock funds the schedules
		// are still in storage,
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched1, sched2]);
...
fn unvested_balance_should_not_transfer() {
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(45));
...
fn vested_balance_should_transfer() {
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		assert_eq!(user1_free_balance, 100); // Account 1 has free balance
									 // Account 1 has only 5 units vested at block 1 (plus 50 unvested)
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(45));
		assert_ok!(Vesting::vest(Some(1).into(), TKN));
...
fn vested_balance_should_transfer_with_multi_sched() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(13u64, 1, sched0, TKN));
		// Total 10*ED locked for all the schedules.
		assert_eq!(Vesting::vesting(&1, TKN).unwrap(), vec![sched0, sched0]);
		...
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(2304));
		assert_ok!(Vesting::vest(Some(1).into(), TKN));
...
fn non_vested_cannot_vest() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		assert!(!<VestingStorage<Test>>::contains_key(4, TKN));
		assert_noop!(Vesting::vest(Some(4).into(), TKN), Error::<Test>::NotVesting);
	});
}
...
fn vested_balance_should_transfer_using_vest_other() {
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		assert_eq!(user1_free_balance, 100); // Account 1 has free balance
									 // Account 1 has only 5 units vested at block 1 (plus 50 unvested)
		assert_eq!(Vesting::vesting_balance(&1, TKN), Some(45));
		assert_ok!(Vesting::vest_other(Some(2).into(), TKN, 1));
		assert_ok!(Balances::transfer_allow_death(Some(1).into(), 2, 55));
	});
}
...
fn non_vested_cannot_vest_other() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		assert!(!<VestingStorage<Test>>::contains_key(4, TKN));
		assert_noop!(Vesting::vest_other(Some(3).into(), TKN, 4), Error::<Test>::NotVesting);
	});
}
...
fn extra_balance_should_transfer() {
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(200));
		assert_ok!(Vesting::vest(Some(2).into(), TKN));
...
fn liquid_funds_should_transfer_with_delayed_vesting() {
	ExtBuilder::default().existential_deposit(256).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting_balance(&12, TKN), Some(user12_free_balance - 256 * 5));
		...
		assert_eq!(Vesting::vesting(&12, TKN).unwrap(), vec![user12_vesting_schedule]);
...
fn vested_transfer_works() {
	ExtBuilder::default().existential_deposit(256).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting(&4, TKN), None);
		...
		assert_ok!(Vesting::do_vested_transfer(3u64, 4, new_vesting_schedule, TKN));
		// Now account 4 should have vesting.
		assert_eq!(Vesting::vesting(&4, TKN).unwrap(), vec![new_vesting_schedule]);
		...
		assert_eq!(Vesting::vesting_balance(&4, TKN), Some(256 * 5));
		...
		assert_eq!(Vesting::vesting_balance(&4, TKN), Some(0));
...
fn vested_transfer_correctly_fails() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![user2_vesting_schedule]);
		...
		assert_noop!(
			Vesting::do_vested_transfer(3u64, 4, new_vesting_schedule_too_low, TKN),
		...
		assert_noop!(
			Vesting::do_vested_transfer(13u64, 4, schedule_per_block_0, TKN),
		...
		assert_noop!(
			Vesting::do_vested_transfer(3u64, 4, schedule_locked_0, TKN),
			Error::<Test>::AmountLow,
		);
...
fn vested_transfer_allows_max_schedules() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		for _ in 0..max_schedules {
			assert_ok!(Vesting::do_vested_transfer(13u64, 4, sched, TKN));
		}

		// The schedules count towards vesting balance
		let transferred_amount =
			<Test as Config>::MinVestedTransfer::get() * max_schedules as Balance;
		assert_eq!(Vesting::vesting_balance(&4, TKN), Some(transferred_amount));
		// and free balance.
		...
		assert_noop!(
			Vesting::do_vested_transfer(3u64, 4, sched, TKN),
...
fn force_vested_transfer_works() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_noop!(
			Vesting::force_vested_transfer(Some(4).into(), TKN, 3, 4, new_vesting_schedule),
			BadOrigin
		);
		...
		// Now account 4 should have vesting.
		assert_eq!(Vesting::vesting(&4, TKN).unwrap()[0], new_vesting_schedule);
		assert_eq!(Vesting::vesting(&4, TKN).unwrap().len(), 1);
		// Ensure the transfer happened correctly.
		let user3_free_balance_updated = Balances::free_balance(&3);
		...
		assert_eq!(Vesting::vesting_balance(&4, TKN), Some(ED * 5));
...
fn force_vested_transfer_correctly_fails() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		let schedule_locked_0 = VestingInfo::new(0, 1, 10);
		assert_noop!(
			Vesting::force_vested_transfer(RawOrigin::Root.into(), TKN, 3, 4, schedule_locked_0),
			Error::<Test>::AmountLow,
		);
...
fn force_vested_transfer_allows_max_schedules() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		// Add max amount schedules to user 4.
		for _ in 0..max_schedules {
			assert_ok!(Vesting::force_vested_transfer(RawOrigin::Root.into(), TKN, 13, 4, sched));
		}
		...
		// Cannot insert a 4th vesting schedule when `MaxVestingSchedules` === 3
		assert_noop!(
			Vesting::force_vested_transfer(RawOrigin::Root.into(), TKN, 3, 4, sched),
			Error::<Test>::AtMaxVestingSchedules,
		);
		// so the free balance does not change.
		assert_eq!(Balances::free_balance(&4), user_4_free_balance);
...
fn merge_schedules_that_have_not_started() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(3u64, 2, sched0, TKN));
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched0]);
		assert_eq!(Balances::usable_balance(&2), 0);
		assert_ok!(Vesting::merge_schedules(Some(2).into(), TKN, 0, 1));
		...
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched1]);
...
fn merge_ongoing_schedules() {
	...
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched2]);
...
fn merging_shifts_other_schedules_index() {
	...
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_eq!(Vesting::vesting(&3, TKN), None);
		...
		assert_ok!(Vesting::do_vested_transfer(4u64, 3, sched0, TKN));
		assert_ok!(Vesting::do_vested_transfer(4u64, 3, sched1, TKN));
		assert_ok!(Vesting::do_vested_transfer(4u64, 3, sched2, TKN));

		// With no schedules vested or merged they are in the order they are created
		assert_eq!(Vesting::vesting(&3, TKN).unwrap(), vec![sched0, sched1, sched2]);
		// and the usable balance has not changed.
		assert_eq!(usable_balance, Balances::usable_balance(&3));

		assert_ok!(Vesting::merge_schedules(Some(3).into(), TKN, 0, 2));
		...
		assert_eq!(Vesting::vesting(&3, TKN).unwrap(), vec![sched1, sched3]);
...
fn merge_ongoing_and_yet_to_be_started_schedules() {
	...
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		Vesting::vest(Some(2).into(), TKN).unwrap();
...
fn merge_finished_and_ongoing_schedules() {
	...
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(3u64, 2, sched2, TKN));
		...
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched2, sched1]);
...
fn merge_finishing_schedules_does_not_create_a_new_one() {
	...
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(3u64, 2, sched1, TKN));
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched1]);
		...
		assert!(!<VestingStorage<Test>>::contains_key(&2, TKN));
...
fn merge_finished_and_yet_to_be_started_schedules() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(13u64, 2, sched1, TKN));
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched1]);
		...
		assert_ok!(Vesting::do_vested_transfer(13u64, 2, sched2, TKN));
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched1, sched2]);
		...
		// locked,
		assert_eq!(Vesting::vesting_balance(&2, TKN), Some(sched1.locked() + sched2.locked()));
...
fn merge_schedules_throws_proper_errors() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_noop!(
			Vesting::merge_schedules(Some(2).into(), TKN, 0, 1),
		...
		assert_eq!(Vesting::vesting(&4, TKN), None);
		assert_noop!(
			Vesting::merge_schedules(Some(4).into(), TKN, 0, 1),
			Error::<Test>::NotVesting
		);

		// There are enough schedules to merge but an index is non-existent.
		Vesting::do_vested_transfer(3u64, 2, sched0, TKN).unwrap();
		assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![sched0, sched0]);
		assert_noop!(
			Vesting::merge_schedules(Some(2).into(), TKN, 0, 2),
			Error::<Test>::ScheduleIndexOutOfBounds
		);

		// It is a storage noop with no errors if the indexes are the same.
		assert_storage_noop!(Vesting::merge_schedules(Some(2).into(), TKN, 0, 0).unwrap());
	});
}
...
fn generates_multiple_schedules_from_genesis_config() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Vesting::vesting(&1, TKN).unwrap(), vec![user1_sched1]);

			let user2_sched1 = VestingInfo::new(1 * ED, 12, 10u64);
			let user2_sched2 = VestingInfo::new(2 * ED, 25, 10u64);
			assert_eq!(Vesting::vesting(&2, TKN).unwrap(), vec![user2_sched1, user2_sched2]);
...
fn vested_transfer_less_than_existential_deposit_fails() {
	ExtBuilder::default().existential_deposit(4 * ED).build().execute_with(|| {
		...
		assert!(
			<Test as Config>::Tokens::minimum_balance(TKN) >
		...
		assert_noop!(Vesting::do_vested_transfer(3u64, 99, sched, TKN), TokenError::BelowMinimum,);
		// force_vested_transfer fails.
		assert_noop!(
			Vesting::force_vested_transfer(RawOrigin::Root.into(), TKN, 3, 99, sched),
...
fn remove_vesting_schedule() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Vesting::do_vested_transfer(3u64, 4u64, new_vesting_schedule, TKN));
		// Now account 4 should have vesting.
		assert_eq!(Vesting::vesting(&4, TKN).unwrap(), vec![new_vesting_schedule]);
		// Account 4 has 5 * 256 locked.
		assert_eq!(Vesting::vesting_balance(&4, TKN), Some(256 * 5));
		// Verify only root can call.
		assert_noop!(Vesting::force_remove_vesting_schedule(Some(4).into(), TKN,  4, 0), BadOrigin);
		// Verify that root can remove the schedule.
		assert_ok!(Vesting::force_remove_vesting_schedule(RawOrigin::Root.into(), TKN, 4, 0));
		// Verify that last event is VestingCompleted.
		System::assert_last_event(Event::VestingCompleted { account: 4, token_id: TKN }.into());
		...
		assert_noop!(
			Vesting::force_remove_vesting_schedule(RawOrigin::Root.into(), TKN, 4, 0),
			Error::<Test>::InvalidScheduleParams
		);
	});
}
...
fn lock_tokens_works() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_ok!(Balances::mint_into(&999, 10000));

		assert_ok!(Vesting::lock_tokens(&999, TKN, 10000, None, 11));

		assert_noop!(
			<Test as Config>::Tokens::ensure_can_withdraw(
				TKN,
				&999,
				1,
		...
		assert_eq!(Vesting::vesting(&999, TKN).unwrap(), vec![VestingInfo::new(10000, 1000, now)]);

		assert_ok!(Balances::mint_into(&999, 10000));

		assert_ok!(Vesting::lock_tokens(&999, TKN, 10000, None, 21));
		...
		assert_eq!(
			Vesting::vesting(&999, TKN).unwrap(),
			vec![VestingInfo::new(10000, 1000, now), VestingInfo::new(10000, 500, now),]
		);
	});
}
...
fn unlock_tokens_works() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		...
		assert_eq!(Vesting::unlock_tokens(&999, TKN, 6000).unwrap().1, 21);

		assert_eq!(
			Vesting::vesting(&999, TKN).unwrap(),
			vec![VestingInfo::new(10000, 1000, now), VestingInfo::new(1500, 100, 6),]
		);

		assert_eq!(VestingInfo::new(1500, 100, 6).locked_at::<Identity>(cur_block), 1500);

		assert_ok!(<Test as Config>::Tokens::ensure_can_withdraw(
			TKN,
			&999,
			13500,
			WithdrawReasons::TRANSFER,
