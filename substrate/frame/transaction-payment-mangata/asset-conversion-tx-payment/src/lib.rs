...
pub(crate) type OnChargeTransactionOf<T> =
	<T as pallet_transaction_payment_mangata::Config>::OnChargeTransaction;
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config:
		frame_system::Config + pallet_transaction_payment_mangata::Config + pallet_asset_conversion::Config
	{
	...
	ChargeAssetIdOf<T>: Send + Sync,
{
	...
	fn validate(
		...
		use pallet_transaction_payment_mangata::ChargeTransactionPayment;
		...
		if let Some((tip, who, initial_payment, asset_id)) = pre {
			match initial_payment {
				InitialPayment::Native(already_withdrawn) => {
					...
					pallet_transaction_payment_mangata::ChargeTransactionPayment::<T>::post_dispatch(
