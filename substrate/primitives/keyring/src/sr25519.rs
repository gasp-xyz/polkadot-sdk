...
impl From<Keyring> for [u8; 32] {
	fn from(k: Keyring) -> Self {
		match k {
			...
			Keyring::Alith =>
				hex2array!("e4843428e3cae5e7df57e719f3e70ebe8270b18961f4c48a7c3ead7b3defcf24"),
			Keyring::Baltathar =>
				hex2array!("aa06f2f531fa023fa9e9db7e13f624b71f962aaa5c30f656021058fd5d528c7e"),
			Keyring::Charleth =>
				hex2array!("08e93c413f8de5630e02fb58de7de0618ebfc1b419e028fd4bcb5e1117c0874b"),
		}
	}
}
...
mod tests {
	...
	fn verify_static_public_keys() {
		Keyring::iter().for_each(|k| println!("{:?}", k.pair().public()) );
		assert!(Keyring::iter().all(|k| { *<Public as AsRef<[u8; 32]>>::as_ref(&k.pair().public()) == <[u8; 32]>::from(k) }));
	}
}
